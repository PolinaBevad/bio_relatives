...
function test_1 {
	...
	BED="$TEST_DIR/resources/genome/compare/bash/correct.bed"
	...
	java -jar $PROJECT_DIR/build/libs/bio_relatives-all-1.4-SNAPSHOT.jar --compare2 $BAM1 $BAM2 $BED -th 4 -m L > $ACTUAL
...
function test_3 {
	...
	BED="$TEST_DIR/resources/genome/compare/bash/correct2.bed"
	...
	java -jar $PROJECT_DIR/build/libs/bio_relatives-all-1.4-SNAPSHOT.jar --compare3 $BAM1 $BAM2 $BAM3 $BED -th 4 -m L  > $ACTUAL
